...
files.forEach(fname => {
    ...
    slideEntries.forEach((entry, sIdx) => {
        ...
        sps.forEach(sp => {
            ...
            const fill = sp[0].match(/<a:solidFill>[\s\S]*?<a:srgbClr val="([^"]+)"/);
        ...
        // Check for shapes whose top-left corner (x, y) is within 0.15 in of another filled shape
        for (let i = 0; i < rectFills.length; i++) {
            for (let j = i + 1; j < rectFills.length; j++) {
                ...
                if (Math.abs(a.x - b.x) < 0.15 && Math.abs(a.y - b.y) < 0.15) {
                    doubleBoxCount++;
                    console.log(`[DOUBLE BOX FLAGGED] ${fname} Slide ${sIdx + 1}: Box A (${a.x}, ${a.y}) vs Box B (${b.x}, ${b.y})`);
                }
            }
        }
    });
...
console.log(`====== DECK-WIDE DOUBLE BOX AUDIT ======`);
console.log(`Total Duplicate Boxes Found Across All 11 Decks: ${doubleBoxCount}`);
